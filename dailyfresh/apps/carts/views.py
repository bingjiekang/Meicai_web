...
class CartInfoView(LoginRequiredMinin,View):
    ...
    def get(self,request):
        ...
        for sku_id, count in cart_dict.items():
            ...
            sku.amount = amount
            # 动态给sku对象增加一个属性count，保存购物车对应的商品
            sku.count = int(count)
            # 添加
            skus.append(sku)
            # 累加计算商品的总数目和总价格
...
# 购物车更新
# /cart/update
class CartUpdateView(LoginRequiredMinin,View):
    '''购物车记录更新'''
    def post(self,request):
        '''记录更新'''
        user = request.user
...
# 使用ajax post请求 获取商品的id
# /cart/delete
class CartDeleteView(LoginRequiredMinin,View):
    '''购物车记录的删除'''
    def post(self,request):
        user = request.user
        if not user.is_authenticated:
